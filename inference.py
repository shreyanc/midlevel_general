...
import utils
from cpresnet import CPResnet, CPResnet_BackProp

import torch

from dataset_utils import normalize_spec
...
default_config = {
    ...
    'load_from_path': 'trained/CPResnet_BackProp_2bf74_2.pt',
    # Path for loading pre-trained midlevel model
...
class InferenceMidlevelModel:
    ...
    def _process_audio_file(self, cache_dir_dataset=None, cache_file_name=None):
        spec_cache_path = utils.get_spectrogram_cache_path(self.audio_fp, self.audio_processor, cache_dir_dataset, cache_file_name)
        if os.path.exists(spec_cache_path):
            try:
                return np.load(spec_cache_path)
            except Exception as e:
                print(f"Could not load {spec_cache_path}: {e}, computing spec")
                return self.audio_processor.process(self.audio_fp).spec
        else:
    ...
    def predict_file(self, audio_fp: str, return_what='midlevels', output_aggregate: str = 'mean', saveto: str = None, cache_dir_dataset=None, cache_file_name=None):
        """
        ...
        spec = self._process_audio_file(cache_dir_dataset=cache_dir_dataset, cache_file_name=cache_file_name)
